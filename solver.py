...
class Solver(object):
    def __init__(self,
        ...
        self.stagewise_loss = getattr(loss_module, self.stagewise_loss)(self.prev_weight, self.curr_weight, self.alpha,
                                                                        self.l_type)
        self._reset()
    ...
    def _val_batch(self, batch_info):
        ...
        # net predict
        batch_est_list = self.net(batch_mix_stft)  # (B,2,F,T)
        # cal stagewise loss
        batch_stagewise_loss = self.stagewise_loss(batch_est_list, batch_target_stft, batch_frame_list)
        # cal metric loss, (B,F,T,2)
        batch_mix_stft = batch_mix_stft.permute(0,3,2,1)  # (B,F,T,2)
        batch_est_stft = batch_est_list[-1].permute(0,2,3,1)  # (B,F,T,2)
        ...
        loss_dict = {}
        loss_dict["mse_loss"] = batch_stagewise_loss.item()
        # create mask
        mask_list = []
    ...
    def _train_batch(self, batch_info):
        ...
        batch_stagewise_loss = self.stagewise_loss(batch_est_list, batch_target_stft, batch_frame_list)
        # params update
        self.update_params(batch_stagewise_loss)
        loss_dict = {}
        loss_dict["stagewise_loss"] = batch_stagewise_loss.item()
        return loss_dict
